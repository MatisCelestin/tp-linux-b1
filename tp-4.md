...
[matis@localhost ~]$ sudo pvcreate /dev/sdb
[sudo] password for matis: 
  Physical volume "/dev/sdb" successfully created.
[matis@localhost ~]$ sudo pvcreate /dev/sdc
  Physical volume "/dev/sdc" successfully created.
[matis@localhost ~]$ sudo pvs
...
[matis@localhost ~]$ sudo vgcreate data /dev/sdb
  Volume group "data" successfully created
[matis@localhost ~]$ sudo vgcreate data /dev/sdc
  A volume group called data already exists.
[matis@localhost ~]$ sudo vgs
  Devices file sys_wwid t10.ATA_VBOX_HARDDISK_VBa67d81f8-f04b7a06 PVID 0Kg7nvJBxcz5ke1cjktlzlhRrYpLHA2F last seen on /dev/sda2 not found.
  VG   #PV #LV #SN Attr   VSize  VFree 
  data   1   0   0 wz--n- <2.00g <2.00g
[matis@localhost ~]$ sudo vgextend data /dev/sdc
  Volume group "data" successfully extended


- créer un nouveau logical volume (LV) : ce sera la partition utilisable :

[matis@localhost ~]$ sudo lvcreate -l 100%FREE data -n  matis
  Logical volume "matis" created.
[matis@localhost ~]$ sudo lvs
[sudo] password for matis: 
  Devices file sys_wwid t10.ATA_VBOX_HARDDISK_VBa67d81f8-f04b7a06 PVID 0Kg7nvJBxcz5ke1cjktlzlhRrYpLHA2F last seen on /dev/sda2 not found.
  LV    VG   Attr       LSize  Pool Origin Data%  Meta%  Move Log Cpy%Sync Convert
  matis data -wi-a----- <2.00g  
...
[matis@localhost ~]$ sudo mkfs -t ext4 /dev/data/matis
...
[matis@localhost ~]$ sudo mkdir /mnt/data1
[matis@localhost ~]$ sudo mount /dev/data/matis /mnt/data1

[matis@localhost dev]$ df -h
...
/dev/mapper/data-matis  2.0G   24K  1.9G   1% /mnt/data1


-définir un montage automatique de la partition (fichier /etc/fstab) :

[matis@localhost ~]$ sudo nano /etc/fstab
[sudo] password for matis: 
(/dev/data/matis /mnt/data1 ext4 defaults 0 0)
[matis@localhost ~]$ sudo umount /mnt/data1
umount: /mnt/data1: not mounted.
[matis@localhost ~]$ sudo mount -av
/                        : ignored
...
[matis@localhost dev]$ sudo dd if=/dev/zero of=/dev/data/matis bs=1M status=progress
1887436800 bytes (1.9 GB, 1.8 GiB) copied, 4 s, 472 MB/s
dd: error writing '/dev/data/matis': No space left on device
2045+0 records in
2044+0 records out
...
[matis@localhost dev]$ df -h /mnt/data1
Filesystem              Size  Used Avail Use% Mounted on
/dev/mapper/data-matis   64Z   64Z  2.0G 100% /mnt/data1
...
[matis@localhost /]$ sudo yum install nfs-utils
[matis@localhost ~]$ sudo mkdir -p /storage/site_web_1
[matis@localhost ~]$ sudo mkdir -p /storage/site_web_2
[matis@localhost ~]$ sudo nano /etc/exports
...
[matis@localhost /]$ sudo apt install nfs-utils
[matis@localhost ~]$ sudo mkdir -p /var/www/site_web_1
[sudo] password for matis: 
[matis@localhost ~]$ sudo mkdir -p /var/www/site_web_2
[matis@localhost ~]$ sudo nano /etc/fstab
...
[matis@localhost ~]$ sudo mount -a
[matis@localhost ~]$ ls -ld /var/www/site_web_1 /var/www/site_web_2
...
```
[matis@localhost ~]$ sudo apt install nginx
...
matis      12517  0.0  0.0   3876  2028 pts/0    S+   21:56   0:00 grep --color=auto nginx

- avec une commande ss, déterminer derrière quel port écoute actuellement le serveur web :
[matis@localhost ~]$ ss -tuln | grep nginx


- en regardant la conf, déterminer dans quel dossier se trouve la racine web :

[matis@localhost ~]$ grep -R "root" /etc/nginx/nginx.conf
        root         /usr/share/nginx/html;
#        root         /usr/share/nginx/html;
[matis@localhost ~]$ ls -l /usr/share/nginx/html
total 12
...
```
[matis@localhost ~]$ sudo firewall-cmd --zone=public --add-port=80/tcp --permanent                  
[sudo] password for matis: 
success
...
```
[matis@localhost ~]$ curl http://192.168.1.100

```
🌞 Vérifier les logs d'accès
```
[matis@localhost ~]$ tail -n 3 /var/log/nginx/access.log
...
[matis@localhost ~]$ ss -tuln | grep 8080
tcp   LISTEN 0      511          0.0.0.0:8080      0.0.0.0:*          
tcp   LISTEN 0      511             [::]:8080         [::]:*          
[matis@localhost ~]$ curl http://192.168.100.1:8080
...
```
[matis@localhost ~]$ sudo useradd -m -d /home/web -s /bin/bash web
[sudo] password for matis: 
[matis@localhost ~]$ sudo passwd web*
...
matis      12742  0.0  0.0   3876  2028 pts/0    S+   22:34   0:00 grep --color=auto nginx

```
🌞 Changer l'emplacement de la racine Web
```
[matis@localhost ~]$ sudo nano /mnt/storage/site_web_1/index.html
[matis@localhost ~]$ sudo nano /etc/nginx/sites-available/default
[matis@localhost ~]$ root /mnt/storage/site_web_1/;
[matis@localhost ~]$  sudo systemctl restart nginx
[matis@localhost ~]$ curl http:10.3.1.11:8080
...
```
[matis@localhost ~]$ grep -R "include.*conf.d" /etc/nginx/nginx.conf
